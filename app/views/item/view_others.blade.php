	...
	<script>
		...
		var item_image_seq = 1;
      	...
      	function zoomInItemImage(item_id) {
           window.open('../item_popup/zoom_in_item_image?item_id='+item_id+'&item_image_seq='+item_image_seq, 'null', 'width=750,height=640,toolbar=no,scrollbars=yes,location=no,resizable=yes');
      	} 
							...
							<td align='center'>
								<table>
									...
									<tr>
										<td align='center' colspan='2'>
											<?
											$is_requested_item = false;

											if(sizeof($trade_items) > 0) {
												for($k = 0; $k < sizeof($trade_items); $k++) {
													if($trade_items[$k]->request_item_id == $my_items[$j]->id) {
											...
											if($is_requested_item) {
											?>
												<a href='#' onclick="cancelTrade('<?=$trade_items[$k]->trade_id?>');" class='btn btn-danger' style='width:78px;'>취소</a>
											<?} else {?>
